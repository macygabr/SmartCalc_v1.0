...
double calc(char* content, double x) {
  ...
  error = conv(string, input);
  for (i = 0; !error && input[i] != '\0'; i++) content[i] = input[i];
  content[i] = '\0';
...
int polish(char* content) {
  ...
  for (int i = 0; content[i] != '\0'; i++) {
    ...
    if (content[i] == '(' || operation(content[i])) {
      ...
      if (content[i] == '(') open++;
      if (operation(content[i]) && i > 0)
        if ((content[i - 1] <= '9' && content[i - 1] >= '0')) error = 1;
    }

    if (operat(content[i])) {
      if (i > 0 && operat(content[i - 1])) error = 1;

      while (last > 0 && content[i] != '^' &&
             mass(steck[last - 1]) >= mass(content[i])) {
        last--;
        res[j++] = steck[last];
        res[j++] = ' ';
      }
    ...
    // printf("content = [%s]\t error = %d\t steck = [%s]\t res = [%s]\n",
    // content, error,steck,res);
    if (content[i] == ')') {
      if (last == 0) error = 1;
      while (last >= 1 && steck[last - 1] != '(' && !error) {
        last--;
        res[j++] = steck[last];
        res[j++] = ' ';
        if (last == 0) error = 1;
...
int conv(char* content, char* str) {
  ...
  for (int i = 0; i < (int)strlen(content); i++, j++) {
    switch (content[i]) {
      case 'c':
        if (content[i + 1] == 'o' && content[i + 2] == 's') {
          str[j] = 'c';
          i += 2;
        } else
          error = 1;
      ...
      case 's':
        if (content[i + 1] == 'i' && content[i + 2] == 'n') {
          str[j] = 's';
          i += 2;
        } else if (content[i + 1] == 'q' && content[i + 2] == 'r' &&
                   content[i + 3] == 't') {
          str[j] = 'q';
          i += 3;
        } else
      ...
      case 't':
        if (content[i + 1] == 'a' && content[i + 2] == 'n') {
          str[j] = 't';
          i += 2;
        } else
          error = 1;
      ...
      case 'l':
        if (content[i + 1] == 'n') {
          str[j] = 'l';
          i++;
        } else if (content[i + 1] == 'o' && content[i + 2] == 'g') {
          str[j] = 'L';
          i += 2;
        } else
          error = 1;
      ...
      case 'm':
        if (content[i + 1] == 'o' && content[i + 2] == 'd') {
          str[j] = 'm';
          i += 2;
        } else
          error = 1;
      ...
      case 'a':
        if (content[i + 1] == 'c' && content[i + 2] == 'o' &&
            content[i + 3] == 's') {
          str[j] = 'C';
          i += 3;
        } else if (content[i + 1] == 's' && content[i + 2] == 'i' &&
                   content[i + 3] == 'n') {
          str[j] = 'S';
          i += 3;
        } else if (content[i + 1] == 't' && content[i + 2] == 'a' &&
                   content[i + 3] == 'n') {
          str[j] = 'T';
          i += 3;
        } else
  ...
  for (int i = 0; str[i] != '\0'; i++)
    if (i > 0 && str[i] == '(' && !operat(str[i - 1]) &&
        !operation(str[i - 1]) && str[i] != '(') {
      error = 1;
    }
  return error;
}
...
int checker(char* content) {
  ...
  for (i = 0; content[i] != '\0'; i++) {
    ...
    if (content[i] == ')') close++;
    if (content[i] == 'm' &&
        ((i == 0 || (content[i - 1] >= '9' && content[i - 1] <= '0')) ||
         ((content[i + 3] >= '9' && content[i + 3] <= '0'))))
      res = 1;
    if (content[i] == 'x' && i > 0 &&
        (content[i - 1] == ')' ||
         (content[i - 1] <= '9' && content[i - 1] >= '0')))
      res = 1;
    if (content[i] == 'x' && (content[i + 1] == '(' ||
                              (content[i + 1] <= '9' && content[i + 1] >= '0')))
      res = 1;
  }
  if (open != close) res = 1;
  if (i > 0 && operat(content[i - 1])) res = 1;
  return res;
}